...
<div class="flex min-h-screen">
    ...
    <aside 
        ...
        class="fixed inset-y-0 left-0 z-40 w-64 shrink-0 bg-navy border-r border-gold/15 flex flex-col transition-transform duration-250 ease-in-out lg:static lg:translate-x-0"
    >
        <!-- Brand Header -->
        <div class="h-20 flex flex-col items-center justify-center border-b border-gold/15 px-6">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            ...
            <p class="px-3 pt-6 mb-2 text-[10px] font-semibold tracking-[0.2em] text-ivory/40 uppercase">Lainnya</p>
            ...
            @auth
                <div class="flex items-center justify-between gap-2 rounded-lg bg-white/5 px-3 py-2.5">
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="shrink-0">
                        @csrf
                        <button type="submit" title="Keluar" class="text-ivory/40 hover:text-rose-400 p-1 transition-colors cursor-pointer">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
    ...
    <div class="flex-1 min-w-0 flex flex-col">
        ...
        <header class="h-20 sticky top-0 z-20 bg-navy/90 backdrop-blur border-b border-gold/15 flex items-center gap-4 px-4 lg:px-8">
            ...
            <div class="flex items-center gap-4 ml-auto">
                ...
                @auth
                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-gold to-gold-dark hidden sm:flex items-center justify-center text-navy font-bold text-xs shadow-sm">
                        {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
                    </div>
                @endauth
            </div>
        ...
        @if(session('success') || session('status') || session('error'))
            <div class="px-4 lg:px-8 pt-4 w-full">
