...
export default function KatalogCard({ katalog, collection }: KatalogCardProps) {
  ...
  return (
    /* SINGLE LEVEL 1 SURFACE */
    <div className="group relative rounded-2xl bg-[#0E1736] border border-white/10 hover:border-[#CBAC70]/40 transition-all duration-300 shadow-md hover:shadow-xl hover:shadow-[#CBAC70]/5 flex flex-col justify-between overflow-hidden">
      
      {/* Visual Cover Image */}
      <Link href={`/katalog/${data.slug}`} className="relative aspect-[16/10] bg-[#070D1F] overflow-hidden block cursor-pointer">
        <img
          src={data.coverImage}
        ...
        {/* Gradient Overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#0E1736] via-[#0E1736]/30 to-transparent opacity-75 group-hover:opacity-55 transition-opacity pointer-events-none" />
      ...
      <div className="p-3.5 sm:p-5 flex flex-col justify-between flex-1 space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <Link href={`/katalog/${data.slug}`} className="block">
            <h3 className="text-sm sm:text-base font-bold text-[#FDFCFF] group-hover:text-[#CBAC70] transition-colors leading-snug">
          ...
          {data.tags && data.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 pt-1">
              {data.tags.map((tag, tIdx) => (
                <span key={tIdx} className="bg-[#14204A] text-[#94A3B8] border border-white/5 text-[9px] font-semibold px-2 py-0.5 rounded-md">
                  #{tag}
                </span>
              ))}
            </div>
