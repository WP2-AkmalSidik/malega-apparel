...
import { X, Trash2, ShoppingBag, ArrowRight, ShieldCheck, ChevronDown, ChevronUp, Check, CheckSquare, Square, MinusSquare } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {cart.length > 0 && (
          <div className="px-4 py-2.5 bg-[#0D183A] border-b border-white/10 flex items-center justify-between text-xs">
              ...
              <button
                ...
                className={`w-4 h-4 rounded flex items-center justify-center transition-all cursor-pointer ${
                  allSelected
                    ...
                    : isIndeterminate
                    ? 'bg-[#CBAC70]/40 border border-[#CBAC70] text-[#CBAC70]'
                    : 'bg-[#070D1F] border border-white/30 group-hover:border-[#CBAC70]'
                }`}
              >
                {allSelected ? (
                  <Check className="w-3 h-3 stroke-[3]" />
                ) : isIndeterminate ? (
                  <span className="w-2 h-0.5 bg-[#CBAC70] rounded-full" />
                ) : null}
              </button>
              <span className="font-bold text-[#FDFCFF] text-xs">
                Pilih Semua <span className="text-[#94A3B8] font-normal">({selectedItems.length}/{cart.length})</span>
              </span>
            </label>
            ...
            {selectedItems.length > 0 && (
              <button
                ...
                className="text-xs text-rose-400 hover:text-rose-300 font-bold flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-rose-500/10 transition-colors cursor-pointer"
                title="Hapus semua item yang dipilih"
              >
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`rounded-xl bg-[#080E20] border transition-all overflow-hidden ${
                    isSelected ? 'border-[#CBAC70]/30 bg-[#080E20]' : 'border-white/5 opacity-70 bg-[#060B18]'
                  }`}
                >
                    ...
                    <div className="pt-2 shrink-0">
                      <button
                        ...
                        className={`w-4 h-4 rounded flex items-center justify-center transition-all cursor-pointer ${
                          isSelected
                            ? 'bg-[#CBAC70] text-[#0B132B]'
                            : 'bg-[#070D1F] border border-white/30 hover:border-[#CBAC70]'
                        }`}
        ...
        {cart.length > 0 && (
          <div className="p-4 sm:p-5 border-t border-white/10 bg-[#080E20] space-y-3.5">
            ...
            <button
              ...
              className="w-full py-3.5 bg-gradient-to-r from-[#E3CD99] via-[#CBAC70] to-[#A58645] hover:opacity-95 text-[#0B132B] rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg transition-all active:scale-98 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
            >
              <span>
                {selectedItems.length > 0
